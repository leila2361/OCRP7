...
from dashboard_plot import boxplot_var_by_target
...
def main():
    ...
    X_tr_all, y_tr_all = get_all_proc_data_tr()  #
    ...
    def get_list_display_features(shap_val_trans, def_n, key):
        ...
        disp_box_cols = st.multiselect(
            'Choose the features to display (default: order of general importance for lgbm calssifier):',
            sorted(all_feat),
...
the values on the arrows is the predicted probability of default on the loan (non repayment).")
            ...
            if st.checkbox('Show detail of the shap values for 10 nearest neighbors (without re-scaling)', key=35):
    ...
    if st.sidebar.checkbox("Customer's data"):
        ...
        if st.checkbox('Show comparison with 10 neighbors and random sample', key=31):
            ...
            df_display = pd.concat([X_cust.loc[disp_cols_not_prepro].rename('cust'),
                                    ...
                                    X_neigh[disp_cols_prepro].mean().rename('10 neigh (mean)'),
...
unprocessed  \n- _cust prepro_: values of the feature for the \
 applicant customer after categorical encoding and standard scaling\
  \n- _10 neigh (mean)_: mean of the preprocessed values of each feature \
...
for a random sample of customers from the training set.")
    ...
    if st.sidebar.checkbox('Boxplots of the main features', key=23):
        ...
        with st.spinner('Boxplot creation in progress...'):
            ...
            fig = boxplot_var_by_target(X_tr_all, y_tr_all, X_neigh, y_neigh,
